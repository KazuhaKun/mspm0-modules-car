...
void Test_All_Modules(void)
{
    ...
    while(1)
    {
        ...
        if (tick % 10 == 0) {
            ...
            // 分时显示不同模块信息
            uint8_t display_mode = (tick / 100) % 3; // 每1秒切换一次显示内容

            switch(display_mode) {
                case 0: // 显示循迹
                    OLED_ShowString(80, 6, (uint8_t*)"Track", 16);
                    Test_Display_LineTracker();
                    break;
                case 1: // 显示IMU
        ...
        // 简单的循迹逻辑，遇到路口停止
        if (g_lineTracker.lineState == LINE_STATE_ALL_LINE) {
